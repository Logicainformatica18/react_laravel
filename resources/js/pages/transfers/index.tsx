            ...
            <thead className="bg-gray-100">
              <tr>
                <th className="px-4 py-2">
                  <input
                    type="checkbox"
                    checked={selectedIds.length === transfers.length}
                    onChange={(e) => {
              ...
              {transfers.map((transfer) => (
                <tr key={transfer.id} className="border-t hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2 text-sm text-gray-700 space-x-2">
                    <button
                      onClick={() => fetchTransfer(transfer.id)}
                  ...
                  <td className="px-4 py-2">
                    <strong>{transfer.sender_firstname} {transfer.sender_lastname}</strong><br />
                    <span className="text-sm text-gray-600">{transfer.sender_email}</span>
                  </td>
                  <td className="px-4 py-2">
                    <strong>{transfer.receiver_firstname} {transfer.receiver_lastname}</strong><br />
                    <span className="text-sm text-gray-600">{transfer.receiver_email}</span>
                  </td>
                  <td className="px-4 py-2">
                    {transfer.file_1 && (
                      <a
                        href={`../../uploads/${transfer.file_1}`}
                        download
        ...
        <div className="flex justify-center mt-6 space-x-2">
          {[...Array(pagination.last_page)].map((_, index) => {
            ...
            return (
              <button
                key={page}
                onClick={() => fetchPage(`/transfers/fetch?page=${page}`)}
