                    ...
                    <DialogContent className="sm:max-w-2xl">
                        ...
                        <div className="grid gap-4 py-4">
                            ...
                            <div key={field} className="grid grid-cols-4 items-center gap-4">
                                ...
                                {field === 'details' ? (
                                <textarea id={field} name={field} value={(formData as any)[field]}
                                    onChange={handleChange} className="col-span-3 border rounded px-3 py-2 text-sm"
                                    rows={3} />
                                ) : (
                                <Input type={field === 'quanty' || field === 'price' ? 'number' : 'text' } id={field}
                                    name={field} value={(formData as any)[field]} onChange={handleChange} min={field
                                    === 'quanty' || field === 'price' ? 0 : undefined} className="col-span-3" />
                                )}
                            </div>
                            ))}
                            ...
                            <div key={field} className="grid grid-cols-4 items-start gap-4">
                                ...
                                <div className="col-span-3">
                                    <div className="flex items-center gap-4">
                                        ...
                                        {previews[field] && (
                                        isImage ? (
                                        <img src={previews[field]!} alt={`preview ${num}`}
                                            className="w-12 h-12 object-cover rounded" />
                                        ) : (
                                        <span
                                            className="text-sm text-gray-600 truncate max-w-[120px]">{previews[field]}</span>
                                        )
                                        )}
                                    </div>
                                    {fileSizeText && (
