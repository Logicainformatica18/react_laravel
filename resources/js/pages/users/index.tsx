...
export default function Users() {
  const { users: initialPagination, roles } = usePage<{
      ...
      <div className="p-8">
        ...
        <div className="overflow-x-auto mt-4">
          <table className="min-w-full divide-y divide-gray-200 bg-white dark:bg-black shadow-md rounded">
            ...
            <tbody>
              {users.map((user) => (
                <tr key={user.id} className="border-t hover:bg-gray-50 dark:hover:bg-gray-700 text-black dark:text-white">
                  ...
                  <td className="px-4 py-2">{user.roles?.map((r) => r.name).join(', ')}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="flex justify-center mt-6 space-x-2">
          {[...Array(pagination.last_page)].map((_, index) => {
            ...
            return (
              <button
                key={page}
                onClick={() => fetchPage(`/users/fetch?page=${page}`)}
